...
Curl.Post = function post(pUrl,headers,body,callback) {
...
Curl.Get = function (path,headers,contents,callback){
//	console.log('Get request!!');
	Curl.Post(path,headers,contents,callback);
}
...
/*	Curl.Get('/admin/account/login',null,function(res){
	...
	Curl.Post('http://cy.com/admin/account/login',PostHead,contents,function(res){
		if(res.statusCode == 302 && res.headers.location=='/admin/index'){
			Curl.Get('http://cy.com/admin/tools/send_notice_crontab',res.headers,null, function(res){
					var body = new Buffer(1024*10);
					var size = 0;
					res.on('data', function (chunk) {
						size+=chunk.length;
